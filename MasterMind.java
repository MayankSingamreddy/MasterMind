...
public class MasterMind {
	...
	private int round;  			// initialize round count

	// Constants
	private final int PEGS_IN_CODE = 4;		// The number of pegs in code
	private final int MAX_GUESSES = 10;		// The number of maximum guesses
	private final int PEG_LETTERS = 6;		// The amount of possible peg letters A-F
											
	private final int ASCII_WRAP = 64;      // ASCII value
	...
	public void run()
	{
		...
		printBoard();
		for(round = 1; round <= MAX_GUESSES && !reveal; round++)
		{
			playTurn();
			printBoard();
		}
		if(round == 11)
			...
			System.out.println("Nice work! You found the master code in " + (round - 1) + " guesses. ");
	}



	 /**
         *  This method executes the turn beginning.
         *
         *  @param none
         *  @return void
         */
	...
	{
		System.out.println("Guess " + round + "\n");
		
		getInput();
		int exact = guesses[round-1].getExactMatches(master);
		int partial = guesses[round-1].getPartialMatches(master);
		
		if (exact == PEGS_IN_CODE || round == MAX_GUESSES) 
			reveal = true;
	}
 ...
 /**
         *  This method creates initial peg values.
         *
         *  @param none
         *  @return void
         */
	...
	{
		int roll = 0; //initialize dice roll value
		char pegLetter = 'A'; //initialize peg value
		for (int i = 0; i < PEGS_IN_CODE; i++) {
			roll = die.roll();
			pegLetter = (char)(roll + ASCII_WRAP);
	 ...
	 /**
         *  This method checks for valid input by user and sets letter for input.
         *
         *  @param none
         *  @return void
         */
	...
	public void getInput()
	{
		...
		boolean badInput = true;
			do {
				userInput = Prompt.getString("Enter the code using (A,B,C,D,E,F). For example, "
			...
			for (int i = 0; i < PEGS_IN_CODE; i++) 
				guesses[round-1].getPeg(i).setLetter(userInput.charAt(i));
		}



	 /**
         *  This method checks input for length.
         *
         *  @param string str (input)
         *  @return   if string is valid
         */

	public boolean isValid(String stringInput)
	{
			if(stringInput.length() != PEGS_IN_CODE)
				return false;
			for(int i = 0; i < PEGS_IN_CODE; i++) {
				if(stringInput.charAt(i) < 'A' || stringInput.charAt(i) > 'F')
					return false;
			}
		return true;
	}
